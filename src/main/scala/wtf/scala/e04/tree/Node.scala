...
object Tree {
  implicit def tree2nodeOption[T](t: Tree[T]): Option[Node[T]] = t match {
...
object Node {
  ...
  def add[T](root: Node[T], newNodeWeight: T, newIndex: Index)(implicit num: Numeric[T]): Node[T] = {
    ...
    val updatedTree = if (newNodeWeight <= value) {
      root.left match {
        case oldNode: Node[T] =>
          replace(oldNode, add(oldNode, newNodeWeight, newIndex))
        case Nil =>
          replace(root, root.copy(left = apply(newNodeWeight, newIndex)))
      }
    } else {
      root.right match {
        case oldNode: Node[T] =>
          replace(oldNode, add(oldNode, newNodeWeight, newIndex))
        case Nil =>
          replace(root, root.copy(right = apply(newNodeWeight, newIndex)))
      }
    }
...
case class Node[T] private[tree](index: Index,
                                 value: T,
                                 left: Tree[T] = Nil,
                                 right: Tree[T] = Nil)(implicit num: Numeric[T]) extends Iterable[Node[T]] with Tree[T] {


  def removeNode(n: Node[T]): Tree[T] = {
    replace(n, Nil)
  }

  def addNodeWithWidth(w: T): Node[T] = {
    add(this, w, maxBy(_.index).index + 1)
  }

  private[tree] def replace(oldValue: Node[T], newValue: Tree[T]): Tree[T] = {
    val newRoot = copy()

    def replace(oldValue: Node[T], newValue: Tree[T], current: Node[T], parentOpt: Option[Node[T]] = None): Option[Tree[T]] = {
      ...
      (parentOpt, updatedNodeOpt) match {
        ...
        case (None, Some(updatedNode)) =>
          Some(updatedNode)
        case (parentOpt, None) =>
          def findLeft = current.left.flatMap(l => replace(oldValue, newValue, l, Some(current)))

          def findRight = current.right.flatMap(r => replace(oldValue, newValue, r, Some(current)))
